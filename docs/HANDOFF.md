...
- **当前主战场**：`seal.html` 的《通神·结印》——MediaPipe 手势小游戏。Lyn 正在手机上实测四个手印的识别手感，**下一步取决于实测反馈**。
...
| `seal.html` + `seal.js` | **《结印》游戏**：摄像头 → HandLandmarker → 手印判定 → 碎片飞回 → 面具复原 | 已上线，等真机反馈 |
| `mudra.js` | 四个手印的几何判别 + 屏幕手位圈（`zones`）。**圈的位置和判别阈值是同一份数据，改一处要同步另一处** | 合成单测 9/9 |
| `mudra-test.html` | 手印判别自检（合成关键点）。**改阈值后必须重跑，全绿才算** | — |
| `summon3d.js` | 结印的奖励层：透明 Three.js 叠在摄像头上，`landShard(i, ghost)` / `fillMissing()` / `complete()` | 稳定 |
...
- ✅ 合成关键点自检 9/9；MediaPipe 线上初始化 4.4s（GPU）；碎片飞回/研究性复原/金光闪 在真实 Chrome 前台标签里截图验证过。
...
1. **按真机反馈调手印**：改 `mudra.js` 阈值 + `zones`，重跑 `mudra-test.html`，push。可能要加"顶尊/相向"更明确的高度分界，或把判定帧数 `HOLD_FRAMES`（seal.js，现 4）调松紧。
