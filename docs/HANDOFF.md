...
- 用户认为旧版摆印后播放奖励不够好玩，授权先做单件神树的持续操控原型；随后明确授权 push，供手机线上实测。
- `seal.html?experiment=tree`：环握并移动牵引 → 相向调掌距，可聚合也可拉散 → 顶尊移动扶正 → 蓄满后主动放低双掌释放。丢手暂停，不算释放。
- `seal.html?experiment=tree&input=touch`：不启用摄像头/识别模型，按住拖动体验同一状态机；扶正蓄满后松开手指释放。原开始页已有试玩链接。
- 新文件 `tree-play.mjs` 负责操控；复用 `seal.js` 摄像头、`summon3d.js` 模型切片。使用现有 AI 神树 GLB；金色连接线和飞鸟是明确标注的幻想演绎。
- `node tools/tree-check.mjs` 检查牵引、可逆聚合、扶正、丢手保护、主动释放。手机识别率与帧率仍需真机验证；此实验的目标是先判断没有结尾动画时，操控本身是否值得重玩。
